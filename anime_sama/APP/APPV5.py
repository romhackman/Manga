...
# ======================================================
def load_domaine():
    # APPV4 est dans APP, JSON dans ND_anime_sama
...
try:
    session.get(URL_HOME, headers=headers_search, timeout=5)
except Exception as e:
    print(f"[ERROR] Impossible de se connecter à {URL_HOME} : {e}")

def fetch_anime_search(query):
    """Recherche les titres sur Anime-sama et met à jour la listbox_search"""
    ...
    try:
        r = session.post(URL_SEARCH, data={"query": query}, headers=headers_search, timeout=5)
        r.raise_for_status()
...
def on_titre_keyrelease(event):
    query = entree_titre.get().strip()
    threading.Thread(target=fetch_anime_search, args=(query,), daemon=True).start()

def on_select_anime(event):
    if listbox_search.curselection():
        index = listbox_search.curselection()[0]
        titre = search_results[index]
